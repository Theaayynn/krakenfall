...
export default function PreLoader() {
  ...
  const [isRemoved, setIsRemoved] = useState(false); // Naya state khud ko hatane ke liye
  ...
  const logs = [
    ...
    "HTTPS://KRAKENFALL.COM/MARINE_FORD/ENCRYPTION_BYPASS/SUCCESS",
    "HTTPS://KRAKENFALL.COM/HAKI/CONQUEROR_WAVE_SCAN/SECTOR_7"
  ];
  ...
  const handleEnter = () => {
    gsap.to(".preloader-container", {
      ...
      duration: 0.8,
      ease: "power2.inOut",
      onComplete: () => setIsRemoved(true) // Animation ke baad element DOM se delete
    });
  };

  if (isRemoved) return null; // Agar remove ho gaya toh kuch nahi dikhana

  return (
    <div className="preloader-container fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-white text-black font-mono tracking-widest select-none">
      ...
      <div className="w-full max-w-5xl px-8 flex justify-between items-center text-[10px] sm:text-xs uppercase absolute top-1/2 -translate-y-1/2">
        ...
        <div className="text-right w-1/2 text-gray-500 truncate">
          {currentLog}
        </div>
      </div>
